...
(function () {
    ...
    style.innerHTML = `
        ...
        .mte-head { text-align: center; margin-bottom: 20px; }
        .mte-head img { height: 45px; margin-bottom: 12px; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.1)); }
        .mte-head h2 { margin: 0; color: var(--mte-dark); font-size: 22px; font-weight: 700; line-height: 1.2; }
        .mte-head p { margin: 5px 0 0; color: var(--mte-gray); font-size: 13px; }
        ...
        /* CHAT WIDGET (LEFT) - FIXED ALIGNMENT */
        .mte-chat {
            position: fixed !important; bottom: 20px !important; left: 20px !important;
            width: 380px !important; background: #fff !important; border-radius: 20px !important;
            z-index: 1000001 !important; box-shadow: 0 10px 40px rgba(0,0,0,0.2) !important;
            overflow: hidden !important; display: flex !important; flex-direction: column !important;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1) !important; border: 1px solid rgba(0,0,0,0.05);
        }
        .mte-chat.mte-minimized { width: 70px !important; height: 70px !important; border-radius: 50% !important; border: 4px solid var(--mte-orange) !important; background: var(--mte-orange) !important; }
        .mte-chat.mte-minimized .mte-chat-header { padding: 0 !important; justify-content: center; width: 100%; height: 100%; }
        .mte-chat.mte-minimized .mte-avatar { background: none; box-shadow: none; border: none; padding: 0; width: 40px; height: 40px; }
        .mte-chat.mte-minimized .mte-agent i, .mte-chat.mte-minimized .mte-avatar img { width: 100%; height: 100%; object-fit: contain; }
        
        .mte-chat-header { background: var(--mte-dark) !important; padding: 16px 20px !important; color: #fff !important; display: flex !important; align-items: center !important; justify-content: space-between !important; cursor: pointer; border-bottom: 2px solid var(--mte-orange); }
        .mte-agent { display: flex !important; align-items: center !important; gap: 12px !important; flex: 1 !important; }
        .mte-avatar { position: relative; width: 44px; height: 44px; background: rgba(255,255,255,0.1); border-radius: 10px; padding: 4px; flex-shrink: 0; }
        .mte-avatar img { width: 100%; height: 100%; object-fit: contain; }
        .mte-online { position: absolute; bottom: -2px; right: -2px; width: 12px; height: 12px; background: #10b981; border: 2px solid var(--mte-dark); border-radius: 50%; }
        
        .mte-info { display: flex !important; flex-direction: column !important; justify-content: center !important; text-align: left !important; }
        .mte-info strong { font-size: 15px !important; font-weight: 700 !important; color: #fff !important; line-height: 1.2 !important; margin-bottom: 2px !important; }
        .mte-info span { font-size: 11px !important; color: var(--mte-gray) !important; line-height: 1 !important; }
        
        .mte-close-chat-btn { background: rgba(255,255,255,0.1) !important; border: none !important; color: #fff !important; font-size: 24px !important; cursor: pointer; width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; line-height: 1; transition: 0.2s; }
        .mte-close-chat-btn:hover { background: var(--mte-orange) !important; }

        .mte-messages { height: 320px !important; padding: 20px !important; overflow-y: auto !important; background: #fdfcfb !important; display: flex; flex-direction: column; gap: 12px; }
        .mte-msg { padding: 12px 16px !important; border-radius: 18px !important; font-size: 14px; max-width: 80% !important; line-height: 1.5; animation: mteFadeUp 0.3s !important; }
        ...
        .mte-chat-footer { padding: 16px; background: #fff; border-top: 1px solid #f0f0f0; display: flex; gap: 10px; align-items: center; }
        .mte-chat-footer input { flex: 1; border: 2px solid #f1f5f9; padding: 12px 16px; background: #f8fafc; border-radius: 25px; font-size: 14px; outline: none; }
        .mte-chat-footer button { width: 44px; height: 44px; background: var(--mte-orange); border: none; border-radius: 50%; color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        ...
        @media (max-width: 768px) {
            ...
            .mte-chat { width: calc(100% - 40px) !important; left: 20px !important; }
            .mte-chat.mte-minimized { left: auto !important; right: 20px !important; }
        }
    `;
    ...
    root.innerHTML = `
        ...
        <div id="mte-chat" class="mte-chat mte-minimized">
            <div class="mte-chat-header" onclick="mte.toggleChat()">
                <div class="mte-agent">
                    <div class="mte-avatar"><img src="${LOGO_URL}" alt="Logo"><span class="mte-online"></span></div>
                    <div class="mte-info"><strong>Expert Assistant</strong><span>Typically replies instantly</span></div>
                </div>
                <button class="mte-close-chat-btn" onclick="event.stopPropagation(); mte.closeChat()">&times;</button>
            </div>
            ...
            <div class="mte-chat-footer">
                ...
                <button id="mte-send" onclick="mte.send()" disabled>➤</button>
            </div>
        </div>
    ...
    window.mte = {
        ...
        closeChat() { document.getElementById('mte-chat').classList.add('mte-minimized'); },
        ...
        botMsg(html) {
            ...
            document.getElementById('mte-messages').scrollTop = 9999;
        },
        ...
        startChat(intent) {
            ...
            setTimeout(() => {
                ...
                document.getElementById('mte-send').disabled = false;
